...
import { fetchMedias, saveMedia, deleteMedia } from '../api/db';
...
export function Cinematheque() {
  ...
  const loadMedias = async () => {
    try {
      ...
      toast.error("Erreur de connexion au serveur local");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    if (saved && !isLoading && mediaList.length === 0) {
      try {
        ...
        if (localData.length > 0) {
          toast.loading("Migration de vos données vers la base locale...", { id: "migration" });
          Promise.all(
            localData.map((media) => saveMedia(media))
          )
            .then(() => {
  ...
  const handleAddMedia = async (newMedia) => {
    ...
    try {
      await saveMedia(mediaWithId);
      setMediaList(prev => [mediaWithId, ...prev]);
